...
ggplot(tsvd.df, aes(x = x, y = h0)) + 
  ...
  theme_classic()
```
```{r bestalpha,echo = F, fig.width=4,fig.height=4,fig.cap="\\label{fig:h0figopt}Finding the optimal value of \alpha by iteration. Optimal solution is $0.03$, truncating eigenvalues of A that are less."}
find_alpha<-function(y,U,S,V,A){
 alpha = seq(0.00001,1,0.001)
 error = c()
 for (i in alpha){
   h0 = tsvd(i,y,U,S,V)
...
best_alpha = res$alpha[which.min(res$error)]
ggplot(res, aes(x = alpha, y = error)) +
 geom_point() +
 geom_vline(xintercept = best_alpha, color = "red")
```
